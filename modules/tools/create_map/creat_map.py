...
def create_lane(data, offset, lane_count, left_lanes, right_lanes):
    ...
    print("lane.id.id: {}".format(lane.id.id))
...
def main():
    ...
    if debug_option:
        with open(temp_csv_file_name, 'w') as csvfile:
            csvfile.write("x,y,x,theta,dist_left,conf_left,dist_right,conf_right,width,ratio,center_x,center_y\n")
            for row in data:
                csvfile.write(
    ...
    print "last_central_lane: {}".format(last_central_lane.id.id)
    poi = POI()
